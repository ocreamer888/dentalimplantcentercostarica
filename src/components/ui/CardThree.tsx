    ...
    <div className={`relative flex flex-col w-full z-10 h-full items-center justify-end ${visibilityClasses}`}>
      <div className='flex flex-col items-center justify-end w-full h-full'>
        <Image
          ...
          priority
          fetchPriority='high'
          className={`object-contain object-bottom w-full h-auto  ${dynamicSizes} sm:landscape:w-full sm:landscape:h-auto px-4 pt-20`}
        />
      </div>
    </div>
  );
...
const CardThree: React.FC<CardThreeProps> = ({
  ...
  return (
    <div
      className={`relative flex flex-col h-auto w-full justify-end items-end text-white rounded-3xl flex-1 overflow-hidden bg-gradient-to-t from-purple-900 via-purple-300/90 to-purple-300/80 backdrop-blur-2xl ${className || ''}`}
      role="region"
